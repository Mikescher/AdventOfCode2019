"use strict";
var AdventOfCode2019_18_1;
(function (AdventOfCode2019_18_1) {
    const DAY = 18;
    const PROBLEM = 1;
    async function run() {
        ...
        for (let i = 0; i < 26; i++)
            await createReachabilityMap(map, String.fromCharCode(i + 97), map.keys[i], 1);
        await map.dump();
    ...
    AdventOfCode2019_18_1.run = run;
    async function createReachabilityMap(map, letter, pos, dumpmode) {
        let quadrant = {};
        for (let i = 0; i < 26; i++) {
            quadrant[String.fromCharCode(i + 97)] = [Math.sign(map.keys[i][0] - map.start[0]), Math.sign(map.keys[i][1] - map.start[1])];
        }
        const no_doors = new Array(26).fill(false);
        map.reachability.set("@>@", [0, no_doors]);
        for (let i = 0; i < 26; i++)
            map.reachability.set(String.fromCharCode(i + 97) + ">" + String.fromCharCode(i + 97), [0, no_doors]);
        let visited_map = {};
        let next = []; // x, y, currently_passed_doors, currently_found_keys
        next.push([pos[0], pos[1], no_doors, ['@']]);
        let counter = 0;
        if (dumpmode === 0 || dumpmode === 1)
            await map.dumpWithFillAndNext(visited_map, next);
        for (;;) {
            let ls = next.slice();
            next = [];
            let updates = 0;
            for (let pos of ls) {
                const x = pos[0];
                const y = pos[1];
                let currently_passed_doors = pos[2];
                let currently_found_keys = pos[3];
                const i = (y * 10000000 + x);
                if (i in visited_map) {
                    if (visited_map[i] < counter && visited_map[i] > 4)
                        throw "loop in map :(";
                    continue;
                }
                ...
                if (map.iswalkable([x - 1, y]) && !(((y) * 10000000 + (x - 1)) in visited_map))
                    next.push([x - 1, y, currently_passed_doors, currently_found_keys]);
                if (map.iswalkable([x + 1, y]) && !(((y) * 10000000 + (x + 1)) in visited_map))
                    next.push([x + 1, y, currently_passed_doors, currently_found_keys]);
                if (map.iswalkable([x, y - 1]) && !(((y - 1) * 10000000 + (x)) in visited_map))
                    next.push([x, y - 1, currently_passed_doors, currently_found_keys]);
                if (map.iswalkable([x, y + 1]) && !(((y + 1) * 10000000 + (x)) in visited_map))
                    next.push([x, y + 1, currently_passed_doors, currently_found_keys]);
            }
            if (dumpmode === 0)
    ...
    async function findPaths(map) {
        let queue = []; // < pos, keys, key_count, path_len, path >
        queue.push(["@", new Array(26).fill(false), 0, 0, ""]);
        ...
        for (let ctr = 1;; ctr++) {
            ...
            seen.add(hskey);
            if (ctr % 100 === 0)
                AdventOfCode.outputConsole("[INTERMED] [" + path_len + "] " + path);
            if (ctr % 500 === 0)
    ...
    function array_insert(array, element) {
    ...
    class AOCMap {
        constructor(g) {
            this.reachability = new Map(); // [x;y] => [length;keys]
            this.grid = g;
        ...
        async dumpWithFill(visited_map) {
            let str = "";
            for (let y = 0; y < this.height; y++) {
                for (let x = 0; x < this.width; x++) {
                    const i = (y * 10000000 + x);
        ...
        async dumpWithFillAndNext(visited_map, next) {
            let str = "";
            for (let y = 0; y < this.height; y++) {
                for (let x = 0; x < this.width; x++) {
                    const i = (y * 10000000 + x);
                    if (next.findIndex(p => p[0] == x && p[1] == y) >= 0)
                        str += "\u25cb";
                    else if (i in visited_map)
                        str += "\u25cf";
                    else {
                        let v = this.get([x, y]);
                        if (v == 46)
...
})(AdventOfCode2019_18_1 || (AdventOfCode2019_18_1 = {}));
