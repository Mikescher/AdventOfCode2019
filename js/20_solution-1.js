"use strict";
var AdventOfCode2019_20_1;
(function (AdventOfCode2019_20_1) {
    const DAY = 20;
    const PROBLEM = 1;
    async function run() {
        ...
        while (dqueue.length > 0) {
            ...
            const [ptx, pty] = pt;
            let ptid = id(pt);
            if (ptid in distmap && distmap[ptid] <= dist)
                continue;
            distmap[ptid] = dist;
            if (grid[pty - 1][ptx] === '.')
                dqueue.push([dist + 1, [ptx, pty - 1]]);
            if (grid[pty][ptx + 1] === '.')
                dqueue.push([dist + 1, [ptx + 1, pty]]);
            if (grid[pty + 1][ptx] === '.')
                dqueue.push([dist + 1, [ptx, pty + 1]]);
            if (grid[pty][ptx - 1] === '.')
                dqueue.push([dist + 1, [ptx - 1, pty]]);
            if (ptid in tunnelsList)
                dqueue.push([dist + 1, tunnelsList[ptid]]);
        }
        await AdventOfCode.outputIntermed(toStr(grid));
        AdventOfCode.output(DAY, PROBLEM, distmap[id(exit)].toString());
    }
    AdventOfCode2019_20_1.run = run;
    ...
    function toStr2(grid, highlights, tunnels) {
        ...
        for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++) {
                if (highlights.some(p => p[1][0] === x && p[1][1] === y))
                    str += "O";
...
})(AdventOfCode2019_20_1 || (AdventOfCode2019_20_1 = {}));
