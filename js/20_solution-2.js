"use strict";
var AdventOfCode2019_20_2;
(function (AdventOfCode2019_20_2) {
    const DAY = 20;
    const PROBLEM = 2;
    async function run() {
        ...
        while (dqueue.length > 0) {
            ...
            let ptid1 = id(pt);
            if (ptid2 in distmap && distmap[ptid2] <= dist)
                continue;
            if (dist !== last_dist) {
            ...
            distmap[ptid2] = dist;
            if (grid[pty - 1][ptx] === '.')
                dqueue.push([dist + 1, depth, [ptx, pty - 1]]);
            if (grid[pty][ptx + 1] === '.')
                dqueue.push([dist + 1, depth, [ptx + 1, pty]]);
            if (grid[pty + 1][ptx] === '.')
                dqueue.push([dist + 1, depth, [ptx, pty + 1]]);
            if (grid[pty][ptx - 1] === '.')
                dqueue.push([dist + 1, depth, [ptx - 1, pty]]);
            if (ptid1 in tunnelsList) {
                ...
                if (depth === 0 && !tunnel_dir) { /* wall */ }
                    ...
                    dqueue.push([dist + 1, depth + (tunnel_dir ? 1 : -1), tunnel_pos]);
            }
        }
    }
    AdventOfCode2019_20_2.run = run;
    function id(p) { return p[1] * 10000 + p[0]; }
    function id2(p, d) { return p[1] * 1000000 + 1000 * p[0] + d; }
    ...
    function toStr2(grid, highlights, tunnels) {
        const width = grid[0].length;
        const height = grid.length;
        let str = "";
        for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++) {
                if (highlights.some(p => p[2][0] === x && p[2][1] === y))
                    str += "O";
                else if (id([x, y]) in tunnels)
                    str += "@";
                else
                    str += grid[y][x];
            }
    ...
    function point_to_str(p) { return `[${p[0]}|${p[1]}]`; }
})(AdventOfCode2019_20_2 || (AdventOfCode2019_20_2 = {}));
