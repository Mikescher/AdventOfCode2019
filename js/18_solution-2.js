"use strict";
var AdventOfCode2019_18_2;
(function (AdventOfCode2019_18_2) {
    const DAY = 18;
    const PROBLEM = 2;
    async function run() {
        ...
        let positions = {};
        for (let k of keys) {
            positions[k] = FindPositionOf(grid, k);
        }
    ...
    AdventOfCode2019_18_2.run = run;
    function CollectKeys(grid, keyPaths, positions, pos) {
        let currentMinimum = Number.MAX_SAFE_INTEGER;
        let startingSet = [pos[0], pos[1], pos[2], pos[3]];
        let q = [];
        ...
        while (q.length > 0) {
            ...
            for (let i = 0; i < 4; i++) {
                for (let k of keyPaths[id(state.Positions[i])]) {
                    ...
                    var newPos = state.Positions.slice();
    ...
    function FindPositionOf(grid, needle) {
    ...
    function ReachableKeys(map, start, currentKeys) {
        let list = [];
        let visited = new Set();
        var q = [];
        var s = [];
...
})(AdventOfCode2019_18_2 || (AdventOfCode2019_18_2 = {}));
